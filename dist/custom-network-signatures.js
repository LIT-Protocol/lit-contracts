  ...
  static from(obj, allowIndexed) {
    ...
    if (typeof obj === "string") {
      ...
      while (obj.length && obj.peekType("BRACKET")) {
        ...
        arrayChildren = new ParamType(_guard2, "", type2, baseType, null, comps, arrayLength, arrayChildren);
      ...
      return new ParamType(_guard2, name2, type2, baseType, indexed2, comps, arrayLength, arrayChildren);
    }
    const name = obj.name;
    ...
    if (arrayMatch) {
      ...
      return new ParamType(_guard2, name || "", type, "array", indexed, null, arrayLength, arrayChildren);
    }
    if (type === "tuple" || type.startsWith("tuple(") || type.startsWith("(")) {
      const comps = obj.components != null ? obj.components.map((c) => ParamType.from(c)) : null;
      const tuple = new ParamType(_guard2, name || "", type, "tuple", indexed, comps, null, null);
      return tuple;
    }
    type = verifyBasicType(obj.type);
    return new ParamType(_guard2, name || "", type, type, indexed, null, null, null);
  }
...
// src/custom-network-signatures.ts
var __dirname = "/Users/anson/Projects/lit-contracts/src", __filename = "/Users/anson/Projects/lit-contracts/src/custom-network-signatures.ts";
function getBaseDirectory(useScriptDirectory = false, callerPath) {
  if (useScriptDirectory) {
    if (callerPath) {
      const callerDir = import_path2.dirname(import_url.fileURLToPath(callerPath));
    ...
    const moduleDir = import_path2.dirname(import_url.fileURLToPath(import.meta.url));
...
function generateAbiSignatures(networkData) {
  const methodsByContract = new Map;
  ...
  networkData.data.forEach((contractGroup) => {
    ...
    if (methodsByContract.has(contractName)) {
      ...
      if (Object.keys(contractMethods).length > 0) {
        ...
        const events = contractGroup.contracts[0].ABI.filter((item) => item.type === "event");
        signatures[contractName] = {
          address,
          methods: Object.fromEntries(Object.entries(contractMethods).map(([methodName, data]) => [
            methodName,
            data.abi
          ])),
          events
...
`);
    fs.writeFileSync(outputPathCjs, `/**
 * Generated Contract Method Signatures for ${networkName}
 * This file is auto-generated. DO NOT EDIT UNLESS YOU KNOW WHAT YOU'RE DOING.
 */
...
if (require.main == require.module) {
  ...
  if (!jsonFilePath) {
    ...
    console.log("Usage: bun run ./src/custom-network-signatures.ts path/to/networkContext.json [custom-network-name]");
    process.exit(1);
  }
